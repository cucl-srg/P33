...
void cli_local_main( void* pport ) {
    ...
    debug_pthread_init( "CLI Local Server", "CLI Local Server" );
    port = *(uint16_t*)pport;
    free( pport );
    ...
    while( !cli_is_time_to_shutdown() ) {
        ...
        if( clientfd == - 1 ) {
            if( errno == EINTR ) {
                debug_println( "Interrupt received, going back to real accept()" );
                continue;
            }

            /* some error */
            debug_println( "Warrning: real accept() returned an error code (%d)",
                           errno );
            continue;
        }
        debug_println( "accepted a real new connection client #%d\n", clientfd );
    ...
    debug_println( "Telling my owner to shutdown (real)" );
    exit( 0 );
}
