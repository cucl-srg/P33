...
static bool cli_send_multi_help( int fd, const char* prefix, int num_args, ... ) {
...
bool cli_send_help_to( int fd, cli_help_t help_type ) {
    switch( help_type ) {
        case HELP_ALL:
...
show ospf topo: displays the current dynamically computed network topology\n" );
          ...
          case HELP_SHOW_VNS:
              return cli_send_multi_help( fd, "\
show vns [lhost, server, topo[logy], user, vhost]: display information about \n\
  the connection to VNS\n",
5,
HELP_SHOW_VNS_LHOST,
HELP_SHOW_VNS_SERVER,
...
show vns lhost: displays the VNS local host address\n" );
            ...
            case HELP_SHOW_VNS_SERVER:
                return 0==writenstr( fd, "\
show vns server: displays the VNS server address\n" );
