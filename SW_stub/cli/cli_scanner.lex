...
/** copies yytext into yylval.string (minus first/last char if QUOTED is TRUE) */
static int copy_yytext( bool quoted );

/**
 * converts a string to an IP address
 * @return TAV_IP token on success, otherwise 0
 */
static int string_to_ip( char* str, addr_ip_t* result_ip );

/**
 * converts a string to a MAC address
 * @return T_MAC_CONST token on success, otherwise 0
 */
static int string_to_mac( char* str, addr_mac_t* result_mac );
...
static int string_to_mac( char* str, addr_mac_t* result_mac ) {
    ...
    if( sscanf( str, "%X:%X:%X:%X:%X:%X",
        ...
        result_mac->octet[0] = mac_octet[0];
        result_mac->octet[1] = mac_octet[1];
        result_mac->octet[2] = mac_octet[2];
        result_mac->octet[3] = mac_octet[3];
        result_mac->octet[4] = mac_octet[4];
        result_mac->octet[5] = mac_octet[5];
        return TAV_MAC;
    }
}
